<script>
	...
	let logo;
	bands.subscribe(bands => {
		if (bands.length) {
			logo = bands[0].logoUrl;
			backgroundColor = bands[0].palette.darkVibrant.background;
		}
	});
...
<div style="background-color: {backgroundColor}; height: {navbarHeight}" class="navbar-wrapper">
	<!-- Logo -->
	{#if logo}
		<Link on:click={closeMobileNav} to="/" class="logo">
			<img src={logo} alt="Korpslogo">
		</Link>
	{:else}
		<a on:click|preventDefault={closeMobileNav} href="/" use:link class="logo">
...
<!-- Normal list of navbarElms -->
	<ul class="navbar-ul">
		...
		<li><a href={navbarElm.href} class="navbar-elm" use:link >{navbarElm.text}</a></li>
		{/each} -->
		{#each navbarElms as navbarElm}
		<li><Link to={navbarElm.href}>{navbarElm.text}</Link></li>
		{/each}
	</ul>
...
<div bind:this={mobileNav} style="top: {navbarHeight};" class="mobile-menu">
		<ul bind:this={mobileLinks} class="mobile-links">
			...
			<!-- {#each navbarElms as navbarElm}
			<li><Link on:click={toggleMobileNav} to={navbarElm.href}>{navbarElm.text}</Link></li>
			{/each} -->
			{#each navbarElms as navbarElm}
			<li class="mobile-link link"><a use:link on:click={toggleMobileNav} class="navbar-elm mobile-menu-text" href={navbarElm.href}>{navbarElm.text}</a></li>
			{/each}
		</ul>
</div>
